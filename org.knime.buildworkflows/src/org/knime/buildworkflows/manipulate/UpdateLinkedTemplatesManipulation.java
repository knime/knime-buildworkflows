...
public class UpdateLinkedTemplatesManipulation implements WorkflowSegmentManipulation {
    ...
    public void apply(final WorkflowSegment workflowSegment) throws IOException, CanceledExecutionException {
        ...
        File tmpDir = FileUtil.createTempDir("metanode");
        WorkflowLoadHelper loadHelper = WorkflowSegment.createWorkflowLoadHelper(true, tmpDir, warning -> {
            NodeLogger.getLogger(WorkflowSegment.class).warn(warning);
        });
        wfm.updateMetaNodeLinks(loadHelper, true, new ExecutionMonitor());
    }
}
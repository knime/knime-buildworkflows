...
final class WorkflowInputNodeParameters implements NodeParameters {
    ...
    static final class InputNodeConfigClassPersistor
        ...
        public InputNodeConfigType load(final NodeSettingsRO nodeSettings, final Integer loadContext)
            throws InvalidSettingsException {
            NodeSettingsRO node = nodeSettings.getNodeSettings(CFG_KEY_INPUT_NODE + loadContext);
            return InputNodeConfigType.getFromValue(node.getString(CFG_KEY_NODE_CONFIG_CLASS, ""));
        }
        ...
        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{CFG_KEY_INPUT_NODE + ARRAY_INDEX_PLACEHOLDER, CFG_KEY_NODE_CONFIG_CLASS}};
        }
    ...
    static final class InputNodeConfigPersistor
        ...
        public String load(final NodeSettingsRO nodeSettings, final Integer loadContext)
            throws InvalidSettingsException {
            NodeSettingsRO node = nodeSettings.getNodeSettings(CFG_KEY_INPUT_NODE + loadContext);
            NodeSettingsRO config = node.getNodeSettings(CFG_KEY_NODE_CONFIG);
            String paramName = config.getString(CFG_KEY_PARAM_NAME, "");
            return paramName;
        }
        ...
        public String[][] getConfigPaths() {
            return new String[][]{
                {CFG_KEY_INPUT_NODE + ARRAY_INDEX_PLACEHOLDER, CFG_KEY_NODE_CONFIG, CFG_KEY_PARAM_NAME}};
        }
    ...
    enum InputNodeConfigType {
            ...
            @Label(value = "None", description = "No input node will be added for this input.")
            NONE("none"), //

            @Label(value = WorkflowInputNodeConfig.NODE_NAME,
                description = "A workflow input node will be added for this input.")
            WORKFLOW_INPUT(WorkflowInputNodeConfig.class.getCanonicalName()), //

            @Label(value = TableInputNodeConfig.NODE_NAME,
                description = "A table input node will be added for this input.")
            TABLE_INPUT(TableInputNodeConfig.class.getCanonicalName()), //

            @Label(value = RowInputNodeConfig.NODE_NAME, description = "A row input node will be added for this input.")
